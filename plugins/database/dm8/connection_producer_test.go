...
func TestInit_clientTLS(t *testing.T) {
	...
	// Create certificates for DM authentication
	...
	// //////////////////////////////////////////////////////
	// Set up DM config file
	rawConf := `
[mysqld]
ssl
ssl-ca=/etc/dm/ca.pem
ssl-cert=/etc/dm/server-cert.pem
ssl-key=/etc/dm/server-key.pem`

	writeFile(t, paths.Join(confDir, "my.cnf"), []byte(rawConf), 0o644)

	// //////////////////////////////////////////////////////
	// Start DM container
	retURL, cleanup := startDMWithTLS(t, "5.7", confDir)
	defer cleanup()
	...
	// //////////////////////////////////////////////////////
	// Test
	dm := newDM(DefaultUserNameTemplate)
	...
	_, err := dm.Init(ctx, conf, true)
	if err != nil {
		t.Fatalf("Unable to initialize dm engine: %s", err)
	}
	...
	client, err := dm.getConnection(ctx)
	if err != nil {
		t.Fatalf("Unable to make connection to DM: %s", err)
	}
	stmt, err := client.Prepare(whoamiCmd)
	if err != nil {
		t.Fatalf("Unable to prepare DM statementL %s", err)
	}
...
func makeTempDir(t *testing.T) (confDir string) {
	confDir, err := ioutil.TempDir(".", "dm-test-data")
	if err != nil {
...
func startDMWithTLS(t *testing.T, version string, confDir string) (retURL string, cleanup func()) {
	...
	runOpts := &dockertest.RunOptions{
		...
		Repository: "dm",
		Tag:        version,
		Cmd:        []string{"--defaults-extra-file=/etc/dm/my.cnf", "--auto-generate-certs=OFF"},
		Env:        []string{fmt.Sprintf("MYSQL_ROOT_PASSWORD=%s", password)},
		// Mount the directory from local filesystem into the container
		Mounts: []string{
			fmt.Sprintf("%s:/etc/dm", confDir),
		},
	}

	resource, err := pool.RunWithOptions(runOpts)
	if err != nil {
		t.Fatalf("Could not start local dm docker container: %s", err)
	}
	resource.Expire(30)
	...
	dsn := fmt.Sprintf("{{username}}:{{password}}@tcp(localhost:%s)/dm", resource.GetPort("3306/tcp"))
	...
	if err != nil {
		cleanup()
		t.Fatalf("Could not connect to dm docker container: %s", err)
	}
...
func connect(t *testing.T, dsn string) (db *sql.DB) {
	...
	err = db.Ping()
	if err != nil {
		t.Fatalf("Failed to ping DM server: %s", err)
	}
...
func setUpX509User(t *testing.T, db *sql.DB, cert certhelpers.Certificate) (username string) {
	...
	cmds := []string{
		...
		fmt.Sprintf("GRANT ALL ON dm.* TO '%s'@'%s' REQUIRE X509", username, "%"),
	}
