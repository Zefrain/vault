...
// dmSQLConnectionProducer implements ConnectionProducer and provides a generic producer for most sql databases
type dmSQLConnectionProducer struct {
...
func (c *dmSQLConnectionProducer) Initialize(ctx context.Context, conf map[string]interface{}, verifyConnection bool) error {
	_, err := c.Init(ctx, conf, verifyConnection)
	return err
}

func (c *dmSQLConnectionProducer) Init(ctx context.Context, conf map[string]interface{}, verifyConnection bool) (map[string]interface{}, error) {
	c.Lock()
	defer c.Unlock()
	...
	// 	dm.RegisterTLSConfig(c.tlsConfigName, tlsConfig)
	// }
	...
	if c.AuthType == connutil.AuthTypeGCPIAM {
		...
		// dialerCleanup, err := registerDriverDM(c.cloudDriverName, c.ServiceAccountJSON)
		if err != nil {
			return nil, err
		}
...
func (c *dmSQLConnectionProducer) Connection(ctx context.Context) (interface{}, error) {
	if !c.Initialized {
...
func (c *dmSQLConnectionProducer) SecretValues() map[string]string {
...
func (c *dmSQLConnectionProducer) Close() error {
	// Grab the write lock
	c.Lock()
	defer c.Unlock()
...
func (c *dmSQLConnectionProducer) getTLSAuth() (tlsConfig *tls.Config, err error) {
...
func (c *dmSQLConnectionProducer) addTLStoDSN() (connURL string, err error) {
...
func (c *dmSQLConnectionProducer) rewriteProtocolForGCP(inDSN string) (string, error) {
	if c.cloudDriverName == "" {
...
// func registerDriver(driverName, credentials string) (cleanup func() error, err error) {
