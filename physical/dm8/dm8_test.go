...
func TestDMPlaintextCatch(t *testing.T) {
	...
	NewDMBackend(map[string]string{
		"address":                      address,
		"database":                     database,
		"table":                        table,
		"username":                     username,
...
func TestDMBackend(t *testing.T) {
	address := os.Getenv("MYSQL_ADDR")
	if address == "" {
		t.SkipNow()
	}
	...
	b, err := NewDMBackend(map[string]string{
		"address":                      address,
		"database":                     database,
		"table":                        table,
		"username":                     username,
...
func TestDMHABackend(t *testing.T) {
	address := os.Getenv("MYSQL_ADDR")
	if address == "" {
		t.SkipNow()
	}
	...
	b, err := NewDMBackend(config, logger)
	if err != nil {
		t.Fatalf("Failed to create new backend: %v", err)
	}
	...
	b2, err := NewDMBackend(config, logger)
	if err != nil {
		t.Fatalf("Failed to create new backend: %v", err)
	}
...
// TestDMHABackend_LockFailPanic is a regression test for the panic shown in
// https://github.com/hashicorp/vault/issues/8203 and patched in
// https://github.com/hashicorp/vault/pull/8229
func TestDMHABackend_LockFailPanic(t *testing.T) {
